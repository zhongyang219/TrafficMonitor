...
- 新增工作列視窗顯示在副顯示器的功能。#2037 #1974 #1952 #1944 #1875 #1870 #1863 #1734
- 多語言代碼重構，多語言字串在外部ini檔中配置，方便程式當地語系化。新增德語、義大利語、俄語、波蘭語、巴西葡萄牙語、土耳其語、希伯來語等語言的支援。
- 更換皮膚介面皮膚清單控制項改為ListBox。
- 選項設置對話方塊中的控制項支援動態調整大小。
- 選項設置對話方塊調整大小時，垂直捲動條不再復位。
- 工作列視窗顯示專案寬度支援動態調整。
- 新增工作列資源佔用圖背景顏色跟隨Windows主題顏色的功能。
- 新增每個皮膚的字體、文本顏色、顯示文本單獨保存的功能。
- 顯示文本設置對話方塊中增加右鍵功能表，允許為單獨的項目恢復默認。
- 崩潰資訊視窗新增調用堆疊的顯示，新增將崩潰資訊寫入日誌的功能。
- Lite版新增了顯卡和硬碟利用率顯示。

**問題修正**

- 修正“常規設置”中更新源沒有保存的問題 #2020 #1561 #1119
- 修正多顯示器DPI不一致時更換皮膚對話方塊預覽圖中字體大小不正確的問題。
- 修正當懸浮窗在不同DPI的顯示器間移動時DPI可能不正確的問題
- 修正顯示了外掛程式專案時工作列內置專案不能全部去掉，會保留一個上傳速度的問題 #1965
- 優化了選項設置中“顏色顯示”控制項中顏色顯示的樣式，修正顏色數量大於8個時控制項右側有明顯的空白的問題
- 修正主視窗和工作列視窗設置中，更改“字體大小”文字方塊中的值後點擊“應用”沒有生效的問題
- 修正工作列視窗滑鼠提示中的網速單位和設置的不一致的問題
- 修正CPU頻率不遵循“數值和單位用空格分隔”選項的問題
- 添加從睡眠狀態恢復時重新初始化連接的處理，解決有時從睡眠狀態恢復時無法獲取到網速的問題。
- 修正皮膚檔中字體名稱有Unicode字元時無法獲取正確的字體的問題，並修正了皮膚檔解析的一些邏輯問題。
- 其他細節方面的改進。
