...
void CTaskBarDlg::ShowInfo(CDC* pDC)
{
	...
	// 获取滚动项目和其他项目
	std::vector<CommonDisplayItem> scrolling_items;
	std::vector<ItemWidthInfo> scrolling_widths;
	std::vector<ItemWidthInfo> other_widths;

	if (CheckScrollModeEnabled())
	{
		scrolling_items = GetScrollingItems();
		for (const auto& item : scrolling_items)
		{
			for (const auto& w : m_item_widths)
			{
				if (w == item)
				{
					scrolling_widths.push_back(w);
					break;
				}
			}
		}
		...
		for (const auto& w : m_item_widths)
		{
			...
			for (const auto& si : scrolling_items)
			{
				if (w == si)
				{
					is_scrolling = true;
	...
	if (IsTasksbarOnTopOrBottom())
	{
		...
		else
		{
			...
			// 绘制滚动项目（第一列）
			if (!scrolling_items.empty())
			{
				int scroll_count = static_cast<int>(scrolling_items.size());
				int display_count = (std::min)(scroll_count, 2);

				// 计算滚动项目的最大宽度
				int scroll_max_width = 0;
				for (const auto& w : scrolling_widths)
				{
					scroll_max_width = (std::max)(scroll_max_width, w.item_width.TotalWidth());
				}

				for (int i = 0; i < display_count; ++i)
				{
					int actual_index = (m_scroll_start_index + i) % scroll_count;
					const auto& item = scrolling_items[actual_index];
					...
					if (i == 0)
					{
						item_rect.SetRect(0, 0, scroll_max_width, TASKBAR_WND_HEIGHT / 2);
					}
					else
					{
						item_rect.SetRect(0, TASKBAR_WND_HEIGHT / 2, scroll_max_width, TASKBAR_WND_HEIGHT);
					}

					// 绘制项目
					if (item.IsPlugin())
						DrawPluginItem(draw, item.PluginItem(), item_rect, item_width.label_width);
					else
						DrawDisplayItem(draw, item.ItemType(), item_rect, item_width.label_width);
				}
			...
			// 绘制其他项目
			int other_index = 0;
			for (const auto& w : other_widths)
			{
				int row = other_index % 2;
				int col = col_offset + (other_index / 2);

				int prev_col_width = 0;
				for (int c = 0; c < col; ++c)
				{
					// 计算前面列的宽度
					if (c == 0 && !scrolling_items.empty())
					{
						for (const auto& sw : scrolling_widths)
						{
							prev_col_width = (std::max)(prev_col_width, sw.item_width.TotalWidth());
						}
					}
					else
					{
						int idx = (c - (scrolling_items.empty() ? 0 : 1)) * 2;
						if (idx >= 0 && idx < static_cast<int>(other_widths.size()))
						{
							int w1 = other_widths[idx].item_width.TotalWidth();
							int w2 = (idx + 1 < static_cast<int>(other_widths.size())) ? other_widths[idx + 1].item_width.TotalWidth() : 0;
							prev_col_width += (std::max)(w1, w2);
						}
					}
					prev_col_width += DPI(theApp.m_taskbar_data.item_space);
				}

				item_rect.left = prev_col_width;
				item_rect.right = item_rect.left + w.item_width.TotalWidth();
				...
				{
					item_rect.top = TASKBAR_WND_HEIGHT / 2;
					item_rect.bottom = TASKBAR_WND_HEIGHT;
				}

				if (w.IsPlugin())
					DrawPluginItem(draw, w.PluginItem(), item_rect, w.item_width.label_width);
				else
					DrawDisplayItem(draw, w.ItemType(), item_rect, w.item_width.label_width);
...
void CTaskBarDlg::DrawDisplayItem(IDrawCommon& drawer, DisplayItem type, CRect rect, int label_width, bool vertical)
{
	...
	// 如果是内存使用率且达到通知条件，将颜色设为红色
	if (type == TDI_MEMORY && theApp.m_general_data.memory_usage_tip.enable)
	{
		if (theApp.m_memory_usage >= theApp.m_general_data.memory_usage_tip.tip_value)
		{
			label_color = RGB(255, 0, 0);    // 红色
			text_color = RGB(255, 0, 0);     // 红色
		}
	}

	// 如果是CPU使用率超过80%，将颜色设为红色
	if (type == TDI_CPU && theApp.m_cpu_usage >= 80)
	{
		label_color = RGB(255, 0, 0);    // 红色
		text_color = RGB(255, 0, 0);     // 红色
	}
...
void CTaskBarDlg::CalculateWindowSize()
{
	...
	if (IsTasksbarOnTopOrBottom())  //任务栏在桌面的顶部或底部时
	{
		...
		else        //非水平排列时，每两个一组排列
		{
			...
			{
				// 滚动模式：滚动项目只占一列宽度
				std::vector<CommonDisplayItem> scrolling_items = GetScrollingItems();
				std::vector<ItemWidthInfo> other_widths;
				...
				for (const auto& w : m_item_widths)
				{
					...
					if (is_scrolling)
					{
						scroll_max_width = max(scroll_max_width, w.item_width.TotalWidth());
					}
				}
				m_window_width = scroll_max_width;

				// 计算其他项目的宽度
				int other_count = static_cast<int>(other_widths.size());
				if (other_count > 0)
				{
					...
					for (const auto& w : other_widths)
					{
						if (index % 2 == 0)
						{
							width0 = w.item_width.TotalWidth();
						}
						else
						{
							m_window_width += max(width0, w.item_width.TotalWidth());
						}
						if (other_count % 2 == 1 && index == other_count - 1)
						{
							m_window_width += w.item_width.MaxWidth();
						}
						index++;
					}

					m_window_width += DPI(theApp.m_taskbar_data.item_space) * ((other_count + 1) / 2);
				}
			}
			else
			{
...
BOOL CTaskBarDlg::OnInitDialog()
{
	...
	if (CheckScrollModeEnabled())
	{
		m_scroll_timer = SetTimer(1001, theApp.m_taskbar_data.scroll_interval, NULL);
	}
...
bool CTaskBarDlg::CheckScrollModeEnabled() const
{
	return theApp.m_taskbar_data.enable_plugin_scroll_mode &&
		!theApp.m_taskbar_data.scroll_plugin_id.empty();
}

std::vector<CommonDisplayItem> CTaskBarDlg::GetScrollingItems() const
{
	std::vector<CommonDisplayItem> items;
	const auto& plugins = theApp.m_plugins.GetPlugins();

	for (const auto& plugin_info : plugins)
	{
		if (plugin_info.plugin != nullptr &&
			plugin_info.Property(ITMPlugin::TMI_NAME) == theApp.m_taskbar_data.scroll_plugin_id)
		{
			for (IPluginItem* item : plugin_info.plugin_items)
			{
				if (item != nullptr &&
					theApp.m_taskbar_data.plugin_display_item.Contains(item->GetItemId()))
				{
...
int CTaskBarDlg::GetScrollingItemCount() const
{
	return static_cast<int>(GetScrollingItems().size());
}

void CTaskBarDlg::UpdateScrollIndex()
{
	int count = GetScrollingItemCount();
	if (count <= 2)
	{
		m_scroll_start_index = 0;
		return;
	}

	m_scroll_start_index = (m_scroll_start_index + 1) % count;
	Invalidate();
}